function [Mov_1_C3,Mov_1_Cz,Mov_1_C4,Mov_2_C3,Mov_2_Cz,Mov_2_C4,Mov_3_C3,Mov_3_Cz,Mov_3_C4,Mov_4_C3,Mov_4_Cz,Mov_4_C4,Mov_5_C3,Mov_5_Cz,Mov_5_C4] = load_subjects_data_cnt(datasetfolder,sub)
...
for Mov = 1:5     % Iterate through each movement type
    for rep = 1:5 % There are five files for each movement type
        ...
        a     = dir([path,'\*.cnt']);                                                     % Get a list of all the data files ending with '.hdf5'
        fname = a(rep,1).name;                                                             % Get one file name at the time 
        str  =[path,'',fname];
        EEG = pop_loadcnt(str,'keystroke','on');
        %% Load events
        ...
        C3 = PreProcess(double(EEG.data(18,:)));
        Cz = PreProcess(double(EEG.data(19,:)));
        C4 = PreProcess(double(EEG.data(20,:)));
        ...
        thr       = 75; % Threshold to reject epochs that are not due to natural EEG amplitudes
